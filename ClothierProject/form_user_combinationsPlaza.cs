...
{
    public partial class form_user_combinationsPlaza : Form
    {

        SqlConnection conn = class_dbconnections.connect;


        public form_user_combinationsPlaza()
        {
            InitializeComponent();
        }

        private void form_user_combinationsPlaza_Load(object sender, EventArgs e)
        {
            ...
            LEFT JOIN cloth_image AS cloth_image5 ON user_outfit_combination.bag_slot = cloth_image5.cloth_id WHERE user_outfit_combination.user_id = " + class_ProgramMaster.logged_user_id + " OR user_outfit_combination.sharing = 1";
        ...
        FlowLayoutPanel addflowlayoutcapsule(int i, string name)
        {
            ...
            itemtokensubmenu.Text = "Add Combination To Favorite";

            itemtokensubmenu.Click += new System.EventHandler(this.addToFavorite);
        ...
        void addToFavorite(object sender, EventArgs e)
        {
            ...
            DataTable controlalreadyfavved = new DataTable();
            string controlsql = "SELECT combination_id FROM user_favorite_combination WHERE user_id = " + class_ProgramMaster.logged_user_id;
            SqlDataAdapter da = new SqlDataAdapter(controlsql, conn);
            da.Fill(controlalreadyfavved);

            if(controlalreadyfavved.Rows.Count == 0)
            {
                SqlCommand addfav = new SqlCommand("INSERT INTO user_favorite_combination(user_id, combination_id) VALUES(@uid, @combid)", conn);
                addfav.Parameters.AddWithValue("@uid", SqlDbType.Int).Value = class_ProgramMaster.logged_user_id;
                addfav.Parameters.AddWithValue("@combid", SqlDbType.Int).Value = combfc_id;
            ...
            {
                MessageBox.Show("This combination is already favved.");
            }
        ...
        private void form_user_combinationsPlaza_MouseDown(object sender, MouseEventArgs e)
        {
        ...
        {
            Form favcombs = new form_user_favcombs();
            favcombs.ShowDialog();
        }
    }
}
